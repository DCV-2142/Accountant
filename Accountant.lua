...
local function Accountant_GetDateKey(t)
	local s = date("%m/%d/%y", t)
	local _, _, mm, dd, yy = string.find(s, "(%d%d)/(%d%d)/(%d%d)")

	if not mm or not dd or not yy then
		return "00000000"
	end

	-- Assumes year to be 2000–2099. Will need fixing otherwise.
	return "20" .. yy .. mm .. dd
end
...
function Accountant_LoadData()
	...
	if Accountant_SaveData[Accountant_Player]["options"]["dateweek"] == nil
		or strlen(Accountant_SaveData[Accountant_Player]["options"]["dateweek"]) ~= 8 then
		Accountant_SaveData[Accountant_Player]["options"]["dateweek"] = Accountant_WeekStart();
	end
	if Accountant_SaveData[Accountant_Player]["options"]["date"] == nil
		or strlen(Accountant_SaveData[Accountant_Player]["options"]["date"]) ~= 8 then
		Accountant_SaveData[Accountant_Player]["options"]["date"] = Accountant_GetDateKey();
	end
end
...
function Accountant_WeekStart()
	...
	return Accountant_GetDateKey(ct)
end
